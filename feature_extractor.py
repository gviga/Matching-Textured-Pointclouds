...
#from render_point_cloud import batch_render
from render_splats import batch_render
...
import random
from utils import convert_splat_container_to_torch_pointcloud

FEATURE_DIMS = 1536 # diffusion unet + dino
...
def get_features_per_vertex(
    ...
    batched_renderings,img,camera, depth = batch_render(
        device, mesh, num_views, H, W, use_normal_map
    )
    ...
    for idx in tqdm(range(len(batched_renderings))):
        ...
        aligned_dino_features = get_dino_features(device, dino_model, Image.fromarray((img[0].cpu().numpy().astype(np.uint8))), grid)
       
        aligned_features = torch.hstack([aligned_dino_features*0.5, aligned_dino_features*0.5])
